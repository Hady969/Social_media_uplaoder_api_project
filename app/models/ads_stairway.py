...
class AdsStairway:
    ...
    def __init__(
        ...
        # Pydantic-safe: do not store image_hash on AdSet if schema disallows it
    ...
    def create_ig_mixed_carousel_ad_creative(
        ...
        Requirements:
        - parent link_data MUST include image_hash (cover)
        - each child attachment MUST include image_hash
          (for video cards, supply thumbnail image_hash)

        child_attachments examples:
          {"name":"Card 1","link":"...","image_hash":"..."}                          # image
          {"name":"Card 2","link":"...","video_id":"...","image_hash":"..."}        # video + thumb
        """
        adset = self.adsets[adset_index]
        ...
        for i, att in enumerate(child_attachments, start=1):
            ...
            # Use first card's image_hash as cover
    ...
    # ---------------- OPTIONAL: UPLOAD LOCAL VIDEO TO AD ACCOUNT ----------------
    def upload_ad_video_to_account(self, adset_index: int, video_path: str) -> str:
        ...
        Useful if you want to create carousel video cards without hosting.
        """
        adset = self.adsets[adset_index]
